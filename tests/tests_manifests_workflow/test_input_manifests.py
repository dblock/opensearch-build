...
class TestInputManifests(unittest.TestCase):
    ...
    def test_files_opensearch(self) -> None:
        ...
        self.assertTrue(os.path.join(InputManifests.manifests_path(), "1.3.0/opensearch-1.3.0.yml") in files)
        self.assertTrue(os.path.join(InputManifests.legacy_manifests_path(), "1.2.1/opensearch-1.2.1.yml") in files)

    def test_files_opensearch_dashboards(self) -> None:
        files = InputManifests.files("opensearch-dashboards")
        self.assertTrue(os.path.join(InputManifests.manifests_path(), "1.3.3/opensearch-dashboards-1.3.3.yml") in files)
        self.assertTrue(os.path.join(InputManifests.legacy_manifests_path(), "1.2.1/opensearch-dashboards-1.2.1.yml") in files)
